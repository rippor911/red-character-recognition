...
1. 更稳的轻量 CNN：使用 depthwise separable block、slot pooling、slot 位置嵌入和 MLP 分类头；默认 `feature_dim/head_hidden_dim=384`，仍保持约 51 万参数的轻量规模。
...
1. 如果 `color_pattern_acc` 或 `target_length_acc` 低，先看 `val_errors.csv` 里的 `red_prob_1...red_prob_5` 和 `pred_color_threshold`，通常需要调整阈值范围、颜色分支、颜色增强或颜色类别权重。颜色类别权重只按训练子集统计，可用 `--no-color-class-weight` 做对照。
2. 如果 `char_slot_acc` 低，先看 `char_slot_1_acc...char_slot_5_acc` 是否集中坏在某个位置，再考虑加大输入宽度、训练轮数或 CNN 宽度；可通过 `--feature-dim` 和 `--head-hidden-dim` 做容量对照。
