...
1. 更稳的轻量 CNN：使用 depthwise separable block、avg/max slot pooling、slot 位置嵌入、5 个 slot 上的局部 1D 卷积上下文混合和 MLP 分类头；默认 `feature_dim/head_hidden_dim=384`，并为 5 个位置使用位置专用分类头，整体仍保持约 191 万参数的轻量规模。可用 `--no-slot-context` 关闭上下文混合，或用 `--slot-pooling avg` 切回约 146 万参数的纯均值池化版本做对照。
...
5. 如果 `char_slot_acc` 或 `char_sequence_acc` 低，先看 `char_slot_1_acc...char_slot_5_acc` 是否集中坏在某个位置，再考虑加大输入宽度、训练轮数、CNN 宽度或字符类别权重；字符类别权重也按位置统计，可通过 `--feature-dim`、`--head-hidden-dim`、`--shared-heads` 和 `--no-char-class-weight` 做容量/分类头/权重对照。
...
8. 如果训练集小样本能记住但验证集不上升，优先减弱增强、调高 dropout/weight decay 或检查 train/val 分布。
9. 如果 `training_history.csv` 中 `val_ema_calibrated_final_exact_acc` 高于 `val_raw_calibrated_final_exact_acc`，说明 EMA 更稳；如果 raw 长期更高，可以临时加 `--no-ema` 做对照实验。
10. 如果验证/测试图像存在轻微水平偏移或缩放差异，默认 TTA 会平均 `0,-2,2` 三个位移和 `1,0.95,1.05` 三个尺度；如果发现 TTA 变慢或无收益，可加 `--no-tta`、`--tta-scales 1` 或调整 `--tta-shifts` 做对照。
